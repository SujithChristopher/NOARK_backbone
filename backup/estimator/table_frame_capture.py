        ...
        while True:
            ...
            if ids is not None:
                ...
                for i in range(len(ids)):
                    ...
                    if success:
                        tvec_dict[m_id] = tvec.flatten()
                        cv2.drawFrameAxes(display_img, self.new_camera_matrix, None, rvec, tvec, length=marker_len * 0.5)

                    # --- Rotation Matrix R Calculation ---
                    if 1 in tvec_dict and 2 in tvec_dict and 3 in tvec_dict:
                        org = tvec_dict[2]          # Origin (Marker 2)
                        ...
                        vynorm = np.cross(vznorm, vxnorm) # Y axis (Right-handed)

                        # Construct the 3x3 Rotation Matrix
                        # Columns are the unit vectors of the new coordinate system
                        R = np.column_stack((vxnorm, vynorm, vznorm))
                        # print(R.shape)
                        if cv2.waitKey(1) & 0xFF == ord('s'):
                            self.ref_mat = R
                            self.ref_tvec = tvec_dict[1]
            ...
            table_vec = self.ref_mat.T @ (tvec_dict[1].reshape(3, 1) - self.ref_tvec.reshape(3, 1))
            print(table_vec.T[0])
