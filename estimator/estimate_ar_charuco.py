...
class RealtimeFisheyeAprilTagTracker:
    ...
    def _load_config(self):
        ...
        _pth = r"/home/sujith/Documents/NOARK_backbone/estimator/charuco_pose/charuco_pose.toml"
        with open(_pth, "r", encoding="utf-8") as f:    
            self.table_reference_frame = toml.load(f)

        # Load the rotation matrix and translation vector from the TOML file
        self.R = np.array(self.table_reference_frame["rotation_matrix"])
        self.tvec = np.array(self.table_reference_frame["tvec"])
