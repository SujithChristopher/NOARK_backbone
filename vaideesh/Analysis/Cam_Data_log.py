...
class MainClass:
    def __init__(self, cam_calib_path, table_calib_path, save_dir=None):
        ...
        # ── CSV Save Path ──────────────────────────────────────────────────────
        # If save_dir is provided, save there; otherwise save in current directory
        if save_dir is not None:
            os.makedirs(save_dir, exist_ok=True)
            self.csv_path = os.path.join(save_dir, "camera_data.csv")
        else:
        ...
        print(f"  📷 Camera CSV: {self.csv_path}")
...
if __name__ == "__main__":
    ...
    main = MainClass(cam_calib_path=cam_path, table_calib_path=table_path, save_dir=args.save_dir)
    main.run()
