...
def read_rigid_body_csv_multi(_pth):
    """
    Read a Motive CSV with one or more rigid bodies.

    _pth: path to the CSV file

    Returns (dict_of_dfs, start_time).
    Keys in dict_of_dfs are the lowercase rigid body names found in the file
    (e.g. 'table', 'noark') plus 'markers' for standalone tracked markers.

    Each rigid-body DataFrame contains:
        frame, seconds,
        ang_x/y/z/w   (quaternion rotation),
        pos_x/y/z     (position),
        mean_marker_err,
        marker_m{n}_x/y/z / marker_m{n}_mq  (per-marker position / quality)

    The 'markers' DataFrame contains:
        frame, seconds,
        {rb_prefix}_m{n}_x/y/z  (standalone tracked marker positions)
    """
    df = pd.read_csv(_pth, skiprows=2, header=None, dtype=str)
    ...
    def _find_idx(lst, item):
        return list(locate(lst, lambda x: x == item))

    _rb_idx = _find_idx(_marker_type, "Rigid Body")
    _rb_marker_idx = _find_idx(_marker_type, "Rigid Body Marker")
    _marker_idx = _find_idx(_marker_type, "Marker")

    # _rb_df row layout (position-based):
    #   0 = Name row, 1 = ID row, 2 = Analysis type, 3 = Axis, 4+ = Data
    _rb_df = df[1:]
    _analysis_type = list(_rb_df.iloc[2].values)
    _names_row = list(_rb_df.iloc[0].values)
    _axis_row = list(_rb_df.iloc[3].values)

    _rotation_ids = set(_find_idx(_analysis_type, "Rotation"))

    # group Rigid Body columns by name
    rb_groups = {}
    for col_idx in _rb_idx:
        name = str(_names_row[col_idx]).lower().strip()
        rb_groups.setdefault(name, []).append(col_idx)

    # group Rigid Body Marker columns by their parent RB name
    rb_marker_groups = {}
    for col_idx in _rb_marker_idx:
        rb_name = str(_names_row[col_idx]).lower().split(":")[0].strip()
        rb_marker_groups.setdefault(rb_name, []).append(col_idx)

    result = {}

    for rb_name, rb_cols in rb_groups.items():
        rot_cols = sorted([i for i in rb_cols if i in _rotation_ids])
        pos_cols = sorted([i for i in rb_cols if i not in _rotation_ids])
        ...
        for i in rot_cols:
            axis = _axis_row[i]
            col_names.append("ang_" + axis.lower() if isinstance(axis, str) else "ang_err")
            col_indices.append(i)

        for i in pos_cols:
            if _analysis_type[i] == "Mean Marker Error":
                col_names.append("mean_marker_err")
            else:
                axis = _axis_row[i]
                col_names.append("pos_" + axis.lower() if isinstance(axis, str) else "pos_err")
            col_indices.append(i)

        for i in rb_marker_groups.get(rb_name, []):
            col_head = str(_names_row[i]).lower().split(":")[1].strip().replace("marker", "").strip()
            axis = _axis_row[i]
            if isinstance(axis, str):
                col_names.append("marker_m" + col_head + "_" + axis.lower())
            else:
                col_names.append("marker_m" + col_head + "_mq")
            col_indices.append(i)

        _data = _rb_df[4:][col_indices].copy()
        _data.columns = col_names
        _data = _data.reset_index(drop=True)
        _data = _data.apply(pd.to_numeric, errors="coerce")
        result[rb_name] = _data

    if _marker_idx:
        m_col_names = ["frame", "seconds"]
        m_col_indices = [0, 1]
        for i in _marker_idx:
            name_raw = str(_names_row[i]).lower()
            if ":" in name_raw:
                rb_prefix = name_raw.split(":")[0]
                marker_num = name_raw.split(":")[1].replace("marker", "").strip()
            else:
                rb_prefix = name_raw
                marker_num = str(i)
            axis = _axis_row[i]
            if isinstance(axis, str):
                m_col_names.append(rb_prefix + "_m" + marker_num + "_" + axis.lower())
            else:
                m_col_names.append(rb_prefix + "_m" + marker_num + "_err")
            m_col_indices.append(i)

        _mdata = _rb_df[4:][m_col_indices].copy()
        _mdata.columns = m_col_names
        _mdata = _mdata.reset_index(drop=True)
        _mdata = _mdata.apply(pd.to_numeric, errors="coerce")
        result["markers"] = _mdata

    return result, st_time


def get_marker_name_multi(val):
    """Column name helper for marker_m{val}_x/y/z in multi-RB sub-DataFrames."""
    return {
        "x": "marker_m" + str(val) + "_x",
        "y": "marker_m" + str(val) + "_y",
        "z": "marker_m" + str(val) + "_z",
    }
