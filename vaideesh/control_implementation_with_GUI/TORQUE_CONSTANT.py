...
AVERAGE_SAMPLES     = 160     # HX711 readings to average per capture
LOG_DIR             = "kt_logs"
# ──────────────────────────────────────────────────────────
...
def capture_torque(sensor_ser, n=AVERAGE_SAMPLES):
    sensor_ser.reset_input_buffer()
    readings = []
    deadline = time.time() + 15.0    # increase to 15s
    while len(readings) < n and time.time() < deadline:
        line = sensor_ser.readline().decode("utf-8", errors="ignore").strip()
        print(f"  RAW: '{line}'")     # ← add this to see what's coming in
        if not line:
            continue
        try:
            readings.append(float(line))
...
def main():
    ...
    for idx, pwm in enumerate(pwm_steps):
       ...
       # Settle
        ...
        sensor_ser.reset_input_buffer()   # ← add this line here
