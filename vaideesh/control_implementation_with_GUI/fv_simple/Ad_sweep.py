...
from teensy_seed_fv import TeensyPort, find_seeed_port
...
MAX_F           = 24.0
T_MIN           = 0.0
HOLD_TIME       = 2       # seconds per force step
REST_TIME       = 4       # seconds of zero-force settling between cycles
N_CYCLES        = 5       # asc+desc repetitions per angle
ANGLE_STEPS     = 7
MAG_LIST        = [5.0, 10.0, 15.0, 24.0]   # ascending then descending per angle
EDGE_MARGIN_DEG = 5.0
WX0, WX1 = -0.50,  0.50
WZ0, WZ1 = -0.80,  0.0
    ...
    def __init__(self, base_dir='csv_data', session_name='session'):
        ...
        self._gui = _StreamWriter(os.path.join(d, 'gui.csv'),
                                  ['timestamp', 'magnitude', 'direction',
                                   ...
                                   'sweep_step', 'sweep_dir', 'sweep_cycle'])
        self._active  = False
        self._stopped = False
...
# ── auto sweep ────────────────────────────────────────────────────────────────
class AutoSweep:
    """For each angle: ascending magnitudes [5→24] then descending [24→5].
    The 24N step appears in both halves; the analysis notebook splits them by
    the time-based step rule (two consecutive same-magnitude steps are separated
    by elapsed time > HOLD_TIME)."""

    def __init__(self, parent, state, teensy, session_dir='csv_data', on_complete=None):
        self.state        = state
        self.teensy       = teensy
    ...
    def start(self):
        ...
        n_rest  = ANGLE_STEPS * N_CYCLES
        n_force = len(self._steps) - n_rest
        total_s = n_force * HOLD_TIME + n_rest * REST_TIME
        print(f'[sweep] {len(self._steps)} entries  '
              f'({ANGLE_STEPS} angles × {N_CYCLES} cycles × {len(MAG_LIST)*2} force steps + {REST_TIME}s rest)  '
    ...
    def stop(self):
        ...
        with self.state.lock:
            ...
            self.state.dir_x       = 0.0
            self.state.dir_z       = 0.0
            self.state.sweep_step  = -1
            self.state.sweep_label = ''
            self.state.sweep_cycle = -1
        if self._events_fh:
    ...
    def _build_grid(self, nx, nz):
        ...
        mg  = math.radians(EDGE_MARGIN_DEG)
        a_s = a1 + math.copysign(mg, d)
        a_e = a3 - math.copysign(mg, d)
        ...
        for theta in angles:
            for cycle in range(N_CYCLES):
                for mag in MAG_LIST:              # ascending: 5 → 24
                    result.append((theta, mag, 'asc', cycle))
                for mag in reversed(MAG_LIST):    # descending: 24 → 5
                    result.append((theta, mag, 'desc', cycle))
                result.append((theta, 0.0, 'rest', cycle))  # 4s zero-force settling
        return result
    ...
    def _advance(self):
        ...
        if self._idx >= len(self._steps):
            ...
            if self._on_complete:
                self._on_complete()
            return
        theta, mag, sweep_dir, cycle = self._steps[self._idx]
        ...
        if sweep_dir == 'rest':
            ...
            with self.state.lock:
                ...
                self.state.dir_x       = 0.0
                self.state.dir_z       = 0.0
                self.state.sweep_step  = self._idx
                self.state.sweep_label = 'rest'
                self.state.sweep_cycle = cycle
            print(f'[sweep] {self._idx+1}/{len(self._steps)}  '
                  f'ang={ang_deg:.1f}  REST {REST_TIME}s  (after cycle {cycle+1}/{N_CYCLES})')
            self._timer.start(int(REST_TIME * 1000))
            return
        ...
        with self.state.lock:
            ...
            self.state.sweep_step  = self._idx
            self.state.sweep_label = sweep_dir
            self.state.sweep_cycle = cycle
        print(f'[sweep] {self._idx+1}/{len(self._steps)}  '
              f'ang={ang_deg:.1f}  mag={mag:.1f}N  ({sweep_dir}  cycle {cycle+1}/{N_CYCLES})')
        self._log_event(self._idx, ang_deg, mag, sweep_dir, cycle,
                        T1, T3, tau1, tau2, sol is not None)
    ...
    def _open_events_log(self):
        ...
        self._events.writerow(['timestamp', 'step', 'target_angle_deg',
                                'target_mag_N', 'sweep_dir', 'cycle',
                                'T1', 'T3', 'tau1', 'tau2', 'feasible'])

    def _log_event(self, step, ang, mag, sweep_dir, cycle, T1, T3, tau1, tau2, feasible):
        if not self._events: return
        self._events.writerow([datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'),
                                step, f'{ang:.3f}', f'{mag:.3f}', sweep_dir, cycle,
                                f'{T1:.4f}', f'{T3:.4f}',
...
class NOARKWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('NOARK Force Validation — ascending/descending sweep')
        self.resize(860, 560)
        ...
        self._sweep = AutoSweep(self, self.state, self._enc,
                                ...
                                on_complete=self._on_sweep_complete)
    ...
    def _on_sweep_complete(self):
        self._rec_state = 'idle'
        self._logger.stop()
        print('[arm] sweep complete — logging stopped, files closed')
    ...
    def _build_ui(self):
        ...
        sv.addWidget(_sec('Sweep progress')); sv.addWidget(_sep())
        self.v_sweep_step = _row(sv, 'step',      '#ffbb00')
        self.v_sweep_dir  = _row(sv, 'direction', '#ffbb00')
    ...
    def _refresh(self):
        ...
        # sweep progress
        if self._sweep.running and self._sweep._idx >= 0:
            idx = self._sweep._idx
            total = len(self._sweep._steps)
            _, _, sdir, cyc = self._sweep._steps[idx]
            self.v_sweep_step.setText(f'{idx+1}/{total}')
            self.v_sweep_dir.setText(f'{sdir}  c{cyc+1}/{N_CYCLES}')
        else:
            self.v_sweep_step.setText('--')
            self.v_sweep_dir.setText('--')
    ...
    def _log_loop(self):
        while self.running:
            ...
            with s.lock:
                ...
                dir_x, dir_z = s.dir_x, s.dir_z
                fm           = s.force_mag
                sweep_step   = s.sweep_step
                sweep_label  = s.sweep_label
                sweep_cycle  = s.sweep_cycle
            if not has_noark:
            ...
            self._logger.log_gui(fm, math.degrees(math.atan2(fm*dir_z, fm*dir_x)),
                                 ...
                                 sweep_step, sweep_label, sweep_cycle)
            time.sleep(0.005)
